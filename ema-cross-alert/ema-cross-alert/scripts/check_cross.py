...
def main():
    ...
    try:
        ...
        sys.exit(0)  # non far fallire il workflow per un problema transitorio

    if "values" not in data:
        # Es. rate limit superato, simbolo errato, key non valida
        print("Risposta API senza dati utili:", data)
        sys.exit(0)

    values = list(reversed(data["values"]))  # ordine cronologico crescente
